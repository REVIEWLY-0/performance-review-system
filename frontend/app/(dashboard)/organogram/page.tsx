    ...
    <div className="relative group/card flex flex-col items-center">
      ...
      <div
        className={[
          ...
          isRoot ? 'border-indigo-400 bg-indigo-50' : 'border-gray-300 bg-white',
          ...
          !isMoving && !isValidTarget && !isEditing ? (isRoot ? '' : 'hover:border-gray-400') : '',
        ].filter(Boolean).join(' ')}
        ...
        {isEditing ? (
          <div className="flex flex-col gap-1">
            <input
              ...
              className="text-sm font-semibold text-center border border-indigo-300 rounded px-2 py-0.5 outline-none focus:ring-1 focus:ring-indigo-500 w-full"
              maxLength={100}
            />
          ...
          <span className={`text-sm font-semibold break-words ${isRoot ? 'text-indigo-800' : 'text-gray-800'}`}>
            {node.title}
          </span>
        )}
      </div>
      ...
      {editMode && !isEditing && !isMoving && movingId === null && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-0.5 bg-white border border-gray-200 rounded-md shadow-sm px-1 py-0.5 opacity-0 group-hover/card:opacity-100 transition-opacity z-10 whitespace-nowrap">
          <button onClick={() => onStartEdit(node)}
            className="text-gray-400 hover:text-indigo-600 px-1 py-0.5 text-xs rounded" title="Rename">✎</button>
          <button onClick={() => onMove(node.id)}
            className="text-gray-400 hover:text-orange-500 px-1 py-0.5 text-xs rounded" title="Move">⇄</button>
          <button onClick={() => onDelete(node.id)}
            className="text-gray-400 hover:text-red-600 px-1 py-0.5 text-xs rounded" title="Delete">×</button>
        </div>
      )}
      ...
      {editMode && !isEditing && !isMoving && movingId === null && (
        <button
          ...
          className="mt-1.5 text-xs text-gray-400 hover:text-indigo-600 border border-dashed border-gray-300 hover:border-indigo-400 hover:bg-indigo-50 px-2 py-0.5 rounded-full transition-colors"
        >
          + child
        </button>
      )}
...
function AddForm({ onSave, onCancel }: { onSave: (title: string) => void; onCancel: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 px-4 py-3 rounded-xl border-2 border-dashed border-indigo-300 bg-indigo-50 min-w-[130px] max-w-[200px] w-max">
      <input
        ref={inputRef}
        value={title}
        ...
        maxLength={100}
        className="text-sm text-center border border-indigo-300 rounded px-2 py-0.5 outline-none focus:ring-1 focus:ring-indigo-500 w-full bg-white"
      />
        ...
        <div className="flex justify-center pt-16">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex gap-8">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-14 w-32 bg-gray-200 rounded-xl animate-pulse" />
              ))}
            </div>
          </div>
        </div>
    ...
    <div className="px-4 py-6 sm:px-0">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">Company Organogram</h1>
          <p className="mt-1 text-sm text-gray-600">Organisational hierarchy</p>
        </div>
      ...
      {isEmpty && (
        <div className="space-y-8">
          <div className="flex flex-col items-center text-center py-6">
            <p className="text-gray-700 font-medium mb-1">No organogram has been set up yet.</p>
            {isAdmin ? (
              ...
              <p className="text-gray-400 text-sm">Your admin hasn't created an organogram yet.</p>
            )}
          </div>
            ...
            <div className="overflow-x-auto">
              <div className="inline-flex min-w-full justify-center">
                <div className="flex flex-col items-center opacity-60">
                  <div className="px-5 py-3 rounded-xl border-2 border-indigo-300 bg-indigo-50 shadow-sm text-sm font-semibold text-indigo-800 whitespace-nowrap">
                  ...
                  <div className="flex items-start">
                    {[
                      ...
                      <div key={label} className="relative flex flex-col items-center px-6">
                        <div className={`absolute top-0 h-px bg-gray-300 ${pos === 'first' ? 'left-1/2 right-0' : pos === 'last' ? 'left-0 right-1/2' : 'inset-x-0'}`} />
                        <div className="w-px h-8 bg-gray-300" />
                        <div className="px-4 py-2.5 rounded-xl border-2 border-gray-300 bg-white shadow-sm text-sm font-semibold text-gray-700 whitespace-nowrap">
                          {label}
                        </div>
                      </div>
                    ))}
