...
export default function EmployeeManagerReviewsPage() {
  ...
  const [managers, setManagers] = useState<EmployeeToReview[]>([]);
  ...
  const loadCycles = async () => {
    try {
      ...
      if (targetId) await loadManagers(targetId);
    } catch (err: any) {
  ...
  const loadManagers = async (cycleId: string) => {
    try {
      setLoadingManagers(true);
      setError('');
      const managerList = await getEmployeesToReview(cycleId);
      setManagers(managerList);
    } catch (err: any) {
      setError(err.message || 'Failed to load manager assignments');
      setManagers([]);
    } finally {
  ...
  const submitted = managers.filter((m) => m.reviewStatus === 'SUBMITTED').length;
    ...
    <div className="flex flex-col" style={{ minHeight: 'calc(100vh - 160px)' }}>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight text-on-surface font-display">Manager Reviews</h1>
          <p className="text-sm text-on-surface-variant">Provide upward feedback to your assigned managers</p>
        </div>
      ...
      {loadingManagers ? (
        ...
        <div className="bg-surface-container-lowest dark:bg-[#131b2e] rounded-2xl p-8 mb-10">
          ...
          <p className="text-base text-on-surface-variant mt-2">
            {pending > 0
              ? `You have ${pending} pending manager review${pending !== 1 ? 's' : ''} to submit.`
              : 'All manager reviews submitted — great work!'}
          </p>
        </div>
      )}
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between px-1">
          <h2 className="text-lg font-semibold text-on-surface tracking-tight font-display">Assigned Managers</h2>
          <span className="text-sm text-on-surface-variant">{total} Manager{total !== 1 ? 's' : ''}</span>
        </div>
        ...
        {loadingManagers ? (
          ...
          <div className="bg-surface-container-lowest dark:bg-[#131b2e] rounded-2xl p-16 flex flex-col items-center text-center">
            <Icon name="manage_accounts" className="text-[44px] text-on-surface-variant mb-3" />
            <p className="font-semibold text-on-surface">No managers assigned</p>
            <p className="text-sm text-on-surface-variant mt-1">You haven't been assigned any managers to review for this cycle.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {managers.map((manager, i) => {
              ...
              const isDraft = manager.reviewStatus === 'DRAFT';
              const isSubmitted = manager.reviewStatus === 'SUBMITTED';
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div>
                      <h3 className="text-base font-semibold text-on-surface leading-tight">{manager.name}</h3>
                      <p className="text-sm text-on-surface-variant">{manager.email}</p>
                      {manager.department && (
                  ...
                  <div className="flex flex-row items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    ...
                    <button
                      onClick={() => router.push(`/employee/reviews/manager/${manager.id}?cycleId=${selectedCycleId}`)}
